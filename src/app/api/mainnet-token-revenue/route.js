// API Route: Get Mainnet Token Revenue (Server-side with API key + caching)
...
async function handler(request) {
  try {
    ...
    console.log('🔍 API: Mainnet token revenue using contract:', { contractAddress, contractName });
    ...
    const revenueFunctions = ['get-sbtc-fee-pool', 'get-fee-pool', 'get-revenue', 'get-total-fees'];
    let revenue = 0;
      ...
      try {
        ...
        if (rawValue) {
          ...
          if (satsValue > 0) {
            ...
            console.log('🔍 Mainnet revenue found:', { revenue, function: funcName });
            break;
          }
        }
      } catch (error) {
        console.log(`Mainnet revenue function ${funcName} failed:`, error.message);
      }
    }

    console.log('💰 Mainnet token revenue:', { revenue, successfulFunction });
    ...
    return NextResponse.json(
      { 
        error: 'Failed to fetch mainnet token revenue',
