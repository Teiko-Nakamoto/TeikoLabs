...
async function handler(request) {
  try {
    ...
    console.log('🔍 API: Total locked tokens using contract:', { contractAddress, contractName });
    ...
    // Get total locked tokens with 15-second caching and API key
    const result = await loggedBlockchainCall(
      'api-total-locked-tokens',
      async () => {
    ...
    const lockedTokens = parseInt(result?.value?.value || result?.value || 0);
    const lockedTokensInWholeUnits = lockedTokens / 100000000; // Convert from micro units

    console.log('🔒 Total locked tokens:', { lockedTokens, lockedTokensInWholeUnits });

    const response = NextResponse.json({
      lockedTokens: lockedTokensInWholeUnits,
      rawLockedTokens: lockedTokens,
      contract: {
